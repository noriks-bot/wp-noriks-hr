...
function gck_render_bundle_selector() {
  ...
  <li><strong>✔ Brzosušeći materijal — savršen za ispod kupaćih</strong></li>
        ...
        <?php if ( ! $show_countdown && ! $gck_no_attrs && ! $gck_single_size && ! has_term( array( 'orto-kompresijske-carape', 'orto-leak-boxers', 'orto-kompresijske-majice', 'orto-kneefix', 'orto-controlpro' ), 'product_cat', $product_id ) ) : ?>
    ...
    <?php if ( $show_countdown && ! $gck_no_attrs && ! $gck_single_size && ! has_term( array( 'orto-leak-boxers', 'orto-kompresijske-majice', 'orto-kneefix', 'orto-controlpro' ), 'product_cat', $product_id ) ) : ?>
...
function gck_order_item_meta( $item, $cart_item_key, $values, $order ) {
    ...
    if ( ! empty( $values['_orto_lines'] ) && is_array( $values['_orto_lines'] ) ) {
        ...
        $item->add_meta_data( 'Odabrani paket', sanitize_text_field( $meta_value ), true );
    }
