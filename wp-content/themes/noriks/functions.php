...
/**
 * --------------------------------------------------------------------------
 *  Klaviyo defensive enqueue (NORIKS fix — 2026-05)
 * --------------------------------------------------------------------------
 *  Issue diagnosed: on the HR site klaviyo.js (the main library) is not being
 *  output by the Klaviyo plugin, even though wck-viewed-product.js is. This
 *  caused all Viewed Product / Active on Site / Identify events to fail
 *  client-side starting ~Mar 18-19 2026.
 *
 *  Root cause: Klaviyo plugin's WPKlaviyoAnalytics::should_enqueue_analytics()
 *  returns false on HR — either because is_checkout() falsely matches on HR
 *  product pages, or because the public API key isn't visible in the HR
 *  language context.
 *
 *  Fix: force-enqueue klaviyo.js directly with the stored public API key
 *  on every front-end page except the real checkout. Idempotent — if the
 *  Klaviyo plugin already enqueued klaviyojs we skip.
 */
add_action( 'wp_enqueue_scripts', function () {
    // Never load on standalone Vigo checkout template (it doesn't even
    // output scripts because it skips wp_head/wp_footer).
    if ( function_exists( 'is_page_template' ) && is_page_template( 'template-vigocheckout.php' ) ) {
        return;
    }

    // Skip if Klaviyo plugin already enqueued the main script — avoid double load.
    global $wp_scripts;
    if ( isset( $wp_scripts->registered['klaviyojs'] ) ) {
        return;
    }

    // Read the public API key the Klaviyo plugin stores in wp_options.
    $opts = get_option( 'klaviyo_settings' );
    $key  = is_array( $opts ) && ! empty( $opts['klaviyo_public_api_key'] )
        ? $opts['klaviyo_public_api_key']
        : '';

    // Hard fallback to the known HR public key in case the option is missing
    // on this language context. Public keys are not secret — they appear in
    // every klaviyo.js URL on the public site.
    if ( '' === $key ) {
        $key = 'VGyhGB';
    }

    $src = '//static.klaviyo.com/onsite/js/' . $key . '/klaviyo.js';
    wp_enqueue_script( 'klaviyojs-noriks', $src, array(), null, true );
}, 0 );

/**
 * Add the async attribute exactly like the Klaviyo plugin does
 * (https://help.klaviyo.com/hc/en-us/articles/115005082927).
 */
add_filter( 'script_loader_tag', function ( $tag, $handle ) {
    if ( 'klaviyojs-noriks' !== $handle ) {
        return $tag;
    }
    // Replace the opening script with an async version.
    return preg_replace( '/<script(?![^>]*\basync\b)/', '<script async', $tag, 1 );
}, 10, 2 );

/**
 * Make sure wck-viewed-product.js (and any other wck-* script) waits for our
 * klaviyojs-noriks handle so the Viewed Product event has `klaviyo.push` ready.
 */
add_action( 'wp_enqueue_scripts', function () {
    global $wp_scripts;
    ...
    foreach ( array( 'wck_viewed_product', 'wck_started_checkout', 'wck_added_to_cart', 'kl-identify-browser' ) as $handle ) {
        if ( isset( $wp_scripts->registered[ $handle ] ) ) {
            $wp_scripts->registered[ $handle ]->deps[] = 'klaviyojs-noriks';
        }
    }
}, 99 );
