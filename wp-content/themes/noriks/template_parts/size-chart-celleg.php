<?php
/**
 * Tablica velicina za NORIKS CelLeg (orto-celleg) — 3D kompresijske tajice.
 * Izvor tabele: noriks_celleg_sizes() v functions/product-type.php (ista kot v akordeonu).
 *
...
<div id="custom-size-chart-modal" class="kwsc" role="dialog" aria-modal="true" aria-labelledby="kwsc-title">
  ...
  <div class="kwsc-body">
    <p class="kwsc-lead">Veličinu birajte prema <strong>opsegu struka i bokova</strong>, ne prema broju traperica. Manja veličina daje jaču kompresiju, veća blaži osjećaj.</p>

    <div class="kwsc-scroll">
      <table class="kwsc-table">
        <thead>
          <tr><th scope="col">Veličina</th><th scope="col">Opseg struka</th><th scope="col">Opseg bokova</th></tr>
        </thead>
        <tbody>
          <?php foreach ( noriks_celleg_sizes() as $r ) : ?>
    ...
    <div class="kwsc-how">
      ...
      <ol>
        <li><strong>Struk</strong> izmjerite na najužem dijelu trupa, malo iznad pupka.</li>
        <li><strong>Bokove</strong> izmjerite preko najšireg dijela stražnjice.</li>
        <li><strong>Odaberite red</strong> u koji ulaze obje mjere; ako su u različitim redovima, ravnajte se prema bokovima.</li>
      </ol>
      <p class="kwsc-note">Traka neka bude vodoravna i pripijena, ali ne stegnuta.</p>
    </div>
  </div>
  </div>
</div>
...
#custom-size-chart-modal.kwsc {
  ...
  font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial, sans-serif; color: #2a1c26;
}
...
.kwsc-bar { display: flex; align-items: center; justify-content: space-between; padding: 15px 22px; border-bottom: 1px solid #f1e3ea; flex: 0 0 auto; }
.kwsc-bar h2 { margin: 0; font-size: 19px; font-weight: 800; color: #2a1c26; }
#close-size-chart-x { font-size: 27px; line-height: 1; font-weight: 700; cursor: pointer; color: #8b7b83; padding: 0 4px; }
#close-size-chart-x:hover { color: #2a1c26; }
.kwsc-body { padding: 20px 22px 26px; overflow-y: auto; flex: 1 1 auto; }
.kwsc-lead { font-size: 14.5px; line-height: 1.6; color: #5d4b57; margin: 0 0 16px; }
.kwsc-scroll { overflow-x: auto; -webkit-overflow-scrolling: touch; }
.kwsc-table { width: 100%; border-collapse: collapse; font-size: 15px; }
.kwsc-table th[scope="col"] { background: #f6e3ee !important; color: #3a2030 !important; font-weight: 800; text-align: left; padding: 12px 14px; white-space: nowrap; border-bottom: 2px solid #a4557f; }
...
.kwsc-table td { padding: 12px 14px; color: #5d4b57; white-space: nowrap; }
.kwsc-table tbody tr { border-bottom: 1px solid #f1e3ea; }
.kwsc-table tbody tr:nth-child(odd) { background: #fbf1f6; }
.kwsc-us { display: block; font-size: 11.5px; font-weight: 600; color: #8b7b83; letter-spacing: .02em; }
.kwsc-how h3 { font-size: 16px; font-weight: 800; margin: 0 0 10px; }
.kwsc-how ol { margin: 0; padding-left: 20px; }
.kwsc-how li { font-size: 14.5px; line-height: 1.6; color: #5d4b57; margin-bottom: 7px; }
