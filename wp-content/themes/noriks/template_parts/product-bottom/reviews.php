...
<section style="background: #f9f9f9; max-width: 1440px;" class="comparison-section comparison-section-gray">
    ...
    padding-right: 10px;" class="comparison-intro comparison-intro-gray ">
      ...
      <h1 style="color:black;     margin-bottom: 4px;">
          ...
          <?php elseif ( noriks_is_type( 'nosilka' ) ): ?>

           Niste jedini u potrazi za nošenjem bez boli u leđima.

          <?php elseif ( noriks_is_type( 'kidsnest' ) ): ?>

           Niste jedini u potrazi za mirnim dječjim snom.

          <?php elseif ( noriks_is_type( 'snore' ) ): ?>

           Niste jedini u potrazi za tihom noći i mirnim jutrom.

          <?php elseif ( noriks_is_type( 'cloud' ) ): ?>

           Niste jedini u potrazi za noći bez bolova u leđima.

          <?php elseif ( noriks_is_type( 'cloath' ) ): ?>

           Niste jedini u potrazi za čistim staklom bez tragova.

          <?php elseif ( noriks_is_type( 'bra' ) ): ?>

           Niste jedini u potrazi za udobnom potporom i uspravnijim držanjem.

          <?php elseif ( noriks_is_type( 'hyd' ) ): ?>

           Niste jedini u potrazi za boljom svakodnevnom hidracijom.
          ...
          <?php elseif ( noriks_is_type( 'dental' ) ): ?>
           ...
           Niste jedini koji žele zubno pomagalo čisto kao prvi dan.

          <?php elseif ( noriks_is_type( 'hairmagic' ) ): ?>

           Niste jedine u potrazi za punijom linijom kose bez odlaska u salon.

          <?php elseif ( noriks_is_type( 'lift' ) ): ?>
          ...
          <?php elseif ( noriks_is_type( 'pre' ) ): ?>

           Niste jedine koje traže položaj za spavanje u kojem trbuščić konačno miruje.

          <?php elseif ( noriks_is_type( 'hug' ) ): ?>

           Niste jedine kojima termofor sklizne čim ustanu sa kauča.

          <?php elseif ( noriks_is_type( 'seal' ) ): ?>

           Niste jedini koji na kraju tjedna bacaju hranu koju su platili.

          <?php elseif ( noriks_is_type( 'sr' ) ): ?>

           Niste jedini kojima je košulja izgužvana prije nego što dođu na posao.

          <?php elseif ( noriks_is_type( 'red' ) ): ?>

           Niste jedini koje noću budi utrnula ruka.
    ...
    <p class="note" style="color: black; margin-top: 0px; margin-bottom: 5px;">
           ...
           Tisuće roditelja već nosi svoje mališane u NORIKS BabyGo nosiljci – slobodne ruke, rasterećena leđa i beba priljubljena uz vas.

           <?php elseif ( noriks_is_type( 'kidsnest' ) ): ?>

           Tisuće roditelja već je zamijenilo obični jastuk NORIKS KidsNestom – tiše noći, disanje kroz nos i san koji stvarno odmara.

           <?php elseif ( noriks_is_type( 'snore' ) ): ?>

           Više od 500.000 prodanih komada u Europi – tiše noći i jutra bez umora.

           <?php elseif ( noriks_is_type( 'cloud' ) ): ?>

           Tisuće ljudi koji spavaju na boku već spava s NORIKS Cloudom – poravnata kralježnica i jutra bez ukočenosti.

           <?php elseif ( noriks_is_type( 'cloath' ) ): ?>

           Više od 138.000 kućanstava već čisti staklo samo vodom – bez sprejeva i bez tragova.

           <?php elseif ( noriks_is_type( 'bra' ) ): ?>

           Tisuće žena već nosi NORIKS BRA – bez žice, s prednjim zakopčavanjem i prekriženom potporom na leđima.

           <?php elseif ( noriks_is_type( 'hyd' ) ): ?>

           Tisuće kupaca svakodnevno puni NORIKS HYD bocu – vodikova voda u tri minute, bilo gdje.
           ...
           Tisuće žena već koristi NORIKS HERS HairMagic+ za prekrivanje sijedih i izrasta – rezultat u manje od dvije minute, bez odlaska u salon.
           ...
           Tisuće trudnica već spava s NORIKS Pre jastukom – potpora trbuščiću i leđima, bez utvrde od pet jastuka.

           <?php elseif ( noriks_is_type( 'hug' ) ): ?>

           Tisuće kupaca već nosi NORIKS Hugger – toplina ostaje uz tijelo, a ruke su slobodne za sve ostalo.

           <?php elseif ( noriks_is_type( 'seal' ) ): ?>

           Tisuće kućanstava već koristi NORIKS ChefSeal – hrana vakuumirana u pet sekundi, bez glomaznog aparata na radnoj plohi.
           ...
           Tisuće kupaca već koristi NORIKS RedRelief – 15 minuta dnevno kod kuće, bez tableta i bez zahvata.
...
<?php 
  ...
  $is_cloath_page     = noriks_is_type( 'cloath', $current_product_id );
  $is_cloud_page      = noriks_is_type( 'cloud', $current_product_id );
  $is_snore_page      = noriks_is_type( 'snore', $current_product_id );
  $is_bra_page        = noriks_is_type( 'bra', $current_product_id );
  $is_hyd_page        = noriks_is_type( 'hyd', $current_product_id );
  ...
  $is_pre_page        = noriks_is_type( 'pre', $current_product_id );
  ...
  $is_snug_page       = noriks_is_type( 'snug', $current_product_id );
  ...
  $rv_fallback_title = $is_relief_page ? 'NORIKS Relief bambusove kompresijske čarape'
                     ...
                     : ( $is_pal_page ? 'NORIKS Pal štap za hodanje'
                     : ( $is_red_page ? 'NORIKS RedRelief terapija crvenim svjetlom'
                     ...
                     : ( $is_hug_page ? 'NORIKS Hugger nosivi termofor'
                     : ( $is_pre_page ? 'NORIKS Pre jastuk za trudnice'
  ...
  if ( $is_relief_page ) {
    ...
    include get_stylesheet_directory() . '/auto_reviews/HR_snug.php';
  } elseif ( $is_kompwom_page ) {
    include get_stylesheet_directory() . '/auto_reviews/HR_kompwom.php';
  } elseif ( $is_pal_page ) {
    include get_stylesheet_directory() . '/auto_reviews/HR_pal.php';
  } elseif ( $is_red_page ) {
    ...
    include get_stylesheet_directory() . '/auto_reviews/HR_sr.php';
  } elseif ( $is_dental_page ) {
    include get_stylesheet_directory() . '/auto_reviews/HR_dental.php';
  } elseif ( $is_hug_page ) {
    include get_stylesheet_directory() . '/auto_reviews/HR_hug.php';
  } elseif ( $is_pre_page ) {
    include get_stylesheet_directory() . '/auto_reviews/HR_pre.php';
    ...
    include get_stylesheet_directory() . '/auto_reviews/HR_hairmagic.php';
  } elseif ( $is_brush_page ) {
  ...
  } elseif ( $is_snore_page ) {
    include get_stylesheet_directory() . '/auto_reviews/HR_snore.php';
  } elseif ( $is_cloud_page ) {
    include get_stylesheet_directory() . '/auto_reviews/HR_cloud.php';
  } elseif ( $is_cloath_page ) {
    include get_stylesheet_directory() . '/auto_reviews/HR_cloath.php';
  } elseif ( $is_bra_page ) {
    include get_stylesheet_directory() . '/auto_reviews/HR_bra.php';
  } elseif ( $is_hyd_page ) {
    include get_stylesheet_directory() . '/auto_reviews/HR_hyd.php';
    ...
    include get_stylesheet_directory() . '/auto_reviews/HR_nosilka.php';
  ...
  function get_wc_product_pool(
      ...
      if ( $product_id ) {
          ...
          $is_nosilka   = noriks_is_type( 'nosilka', $product_id );
          $is_kidsnest  = noriks_is_type( 'kidsnest', $product_id );
          if ( $is_ortopas || $is_bunion || $is_fisiorest || $is_norikshers || $is_jastuk || $is_nosilka || $is_kidsnest ) { $is_nogavice = false; }
      }
...
$is_cloath    = ( function_exists('noriks_is_type') && noriks_is_type('cloath') );
$is_cloud     = ( function_exists('noriks_is_type') && noriks_is_type('cloud') );
$is_snore     = ( function_exists('noriks_is_type') && noriks_is_type('snore') );
$is_bra       = ( function_exists('noriks_is_type') && noriks_is_type('bra') );
$is_hyd       = ( function_exists('noriks_is_type') && noriks_is_type('hyd') );
...
$kidsnest_faq = array(
  array( 'questioon' => 'Koliko brzo ću vidjeti da disanje na usta prestaje?', 'answer' => 'Većina roditelja primjećuje tiše disanje i manje buđenja s otvorenim ustima unutar prvih 5–7 noći. Do 14. noći kod većine djece hrkanje se smiruje, a usnice ostaju zatvorene. Punu razliku — vidljivo bolji položaj i mirniji san — roditelji najčešće opisuju oko 21. do 30. dana. Koristite ga svaku noć.' ),
...
$nosilka_faq = array(
  array( 'questioon' => 'Od koje dobi mogu koristiti BabyGo nosiljku?', 'answer' => 'Nosiljka je namijenjena bebama i maloj djeci koja samostalno drže glavicu. Dijete uvijek nosite u preporučenom položaju, priljubljeno uz tijelo, i redovito provjeravajte njegov položaj.' ),
  array( 'questioon' => 'Do koje težine djeteta je nosiljka predviđena?', 'answer' => 'BabyGo je izrađena od čvrste tkanine s pojačanim šavovima i podnosi težinu male djece. Točan raspon dobi i težine naveden je u informacijama o proizvodu — pridržavajte se preporuka.' ),
  array( 'questioon' => 'Hoće li me boljeti leđa i ramena od nošenja?', 'answer' => 'Nosiljka ravnomjerno raspoređuje težinu djeteta preko ramena i leđa, umjesto da sve ide na ruke. Zato većina roditelja može nositi dijete znatno dulje bez boli i naprezanja.' ),
  array( 'questioon' => 'Mogu li obavljati stvari dok nosim dijete?', 'answer' => 'Da — u tome je poanta. Ruke su vam slobodne pa možete kupovati, kuhati, pospremati ili prošetati, dok je dijete sigurno priljubljeno uz vas.' ),
  array( 'questioon' => 'Kako se nosiljka pere?', 'answer' => 'Tkanina se može prati — preporučujemo pranje na nižoj temperaturi i sušenje na zraku, da nosiljka zadrži oblik i čvrstoću.' ),
  array( 'questioon' => 'Mogu li je dijeliti s partnerom?', 'answer' => 'Da. Nosiljka se brzo i jednostavno prilagodi, pa je mogu koristiti mama i tata bez kompliciranog podešavanja.' ),
  array( 'questioon' => 'Postoji li jamstvo povrata novca?', 'answer' => 'Da, vrijedi naše jamstvo povrata novca. Ako niste zadovoljni, kontaktirajte nas i riješit ćemo to.' ),
);
...
// NORIKS BRA (orto-bra) — product FAQ za "Informacije o Proizvodu".
// NORIKS SNORE (orto-snore) — product FAQ za "Informacije o Proizvodu".
$snore_faq = array(
  array(
    'questioon' => 'Kako udlaga smanjuje hrkanje?',
    'answer'    => 'Nježno pomiče donju čeljust prema naprijed. Time se dišni put iza jezika otvara, zrak prolazi slobodno i tkivo prestaje vibrirati — a upravo ta vibracija je zvuk hrkanja.',
  ),
  array(
    'questioon' => 'Kako je prilagodim svojim ustima?',
    'answer'    => 'Uronite je u vruću vodu na nekoliko sekundi da omekša, stavite u usta i ugrizite. Ohladite je hladnom vodom. Ako dosjed nije dobar, postupak možete ponoviti.',
  ),
  array(
    'questioon' => 'Čemu služi pet traka?',
    'answer'    => 'Trake #1–#5 određuju koliko je čeljust pomaknuta naprijed — svaka sljedeća pomiče je 1,5 mm više. Počnite s najmanjim pomakom i povećavajte ga dok hrkanje ne prestane, a čeljust je i dalje udobna.',
  ),
  array(
    'questioon' => 'Hoće li mi boljeti čeljust?',
    'answer'    => 'Prvih nekoliko noći moguć je osjećaj napetosti, kao kod svake udlage. Ako traje, vratite se na traku s manjim pomakom. Trake raspoređuju pritisak po cijelom zubnom luku, pa nema pojedinačnih bolnih točaka.',
  ),
  array(
    'questioon' => 'Mogu li piti dok je nosim?',
    'answer'    => 'Da. Okvir ostavlja usta pokretnima, pa možete piti vodu i govoriti dok je udlaga u ustima.',
  ),
  array(
    'questioon' => 'Kako se čisti?',
    'answer'    => 'Isperite je mlakom vodom nakon svake upotrebe i po potrebi očistite mekom četkicom bez paste. Ne koristite vrelu vodu ni perilicu — visoka temperatura bi joj promijenila oblik.',
  ),
  array(
    'questioon' => 'Odgovara li svima?',
    'answer'    => 'Okvir se oblikuje po vašem ugrizu i odgovara većini veličina usta. Nije namijenjena osobama s protezom, mostovima ili tek ugrađenim implantatima — u tom slučaju se posavjetujte sa stomatologom.',
  ),
  array(
...
// NORIKS Cloud (orto-cloud) — product FAQ za "Informacije o Proizvodu".
$cloud_faq = array(
  array(
    'questioon' => 'Što ako mi jastuk ne odgovara?',
    'answer'    => 'Imate 60 noći da ga isprobate. Ako niste zadovoljni, javite se našoj podršci i vraćamo vam cijeli iznos — bez uvjeta i bez papirologije.',
  ),
  array(
    'questioon' => 'Kako se jastuk pravilno koristi?',
    'answer'    => 'Legnite na bok, postavite jastuk između koljena i traku zategnite oko gornje noge. Kukovi, zdjelica i kralježnica trebaju biti u ravnoj liniji. Traku možete i skinuti te jastuk koristiti pod gležnjeve ili pod vrat.',
  ),
  array(
    'questioon' => 'Ostaje li stvarno na mjestu kad se okrenem?',
    'answer'    => 'Da. Podesiva traka s drukerima ide oko noge i drži jastuk uz koljeno, pa ne ispada kao obični jastuk. Traka ima dva položaja zategnutosti, prema opsegu noge.',
  ),
  array(
    'questioon' => 'Od čega je jastuk napravljen?',
    'answer'    => 'Jezgra je od memorijske pjene medicinske kvalitete s perforacijama koje osiguravaju protok zraka. Navlaka je od prozračne tkanine, skida se i pere. Materijali su certificirani po OEKO-TEX® STANDARD 100 i CertiPUR-EU.',
  ),
  array(
    'questioon' => 'Je li pretvrd ili premekan?',
    'answer'    => 'Srednje je čvrstoće — dovoljno da drži razmak nogu cijelu noć, ali se pod pritiskom prilagodi obliku koljena. Ne splošti se i vraća se u oblik nakon ustajanja.',
  ),
  array(
    'questioon' => 'Kako se pere?',
    'answer'    => 'Navlaku skinite i perite u stroju na 40 °C, bez omekšivača i bez izbjeljivača, sušite na zraku. Pjenu ne perite u stroju — po potrebi je obrišite vlažnom krpom i ostavite da se osuši.',
  ),
  array(
    'questioon' => 'Koliko treba da se naviknem?',
    'answer'    => 'Većina korisnika razliku osjeti već prvu ili drugu noć. Ako ste navikli spavati sa spojenim koljenima, prvih nekoliko noći razmak može biti neobičan — tijelo se obično prilagodi unutar tjedan dana.',
  ),
  array(
    'questioon' => 'Je li prikladan u trudnoći?',
    'answer'    => 'Jest. Preporučeni položaj u trudnoći je spavanje na lijevoj strani, a jastuk između koljena rasterećuje kukove i donji dio leđa. Ako imate zdravstvenih komplikacija, prije uporabe se posavjetujte s liječnikom.',
  ),
);

// Polar NORIKS Cloth (orto-cloath) — product FAQ za "Informacije o Proizvodu".
$cloath_faq = array(
  array(
  ...
  array(
    ...
    'answer'    => 'Od guste mikrovlaknaste tkanine s dvostranim dizajnom: čupava strana skuplja prljavštinu i vodu, a mrežasta strana polira do sjaja. Rub je ojačan mekanom trakom koja ne grebe.',
  ),
  array(
    'questioon' => 'Zašto je skuplja od običnih krpa?',
    'answer'    => 'Zato što je gušća i teža od standardne mikrofibre — upija višestruko više vode, ne pušta vlakna i izdrži stotine pranja. Jedna krpa zamijeni cijeli niz jeftinih koje se nakon nekoliko pranja počnu parati.',
  ),
  array(
    'questioon' => 'Koliko dugo traje u odnosu na obične krpe?',
    'answer'    => 'Uz pravilno održavanje izdrži stotine upotreba. Obične krpe iz dućana obično nakon 20-ak pranja izgube gustoću i počnu ostavljati tragove.',
  ),
  array(
    'questioon' => 'Ostavlja li tragove ili mrlje?',
    'answer'    => 'Ne. Zbog gustoće i dvostranog dizajna voda se upija umjesto da se razmazuje, pa staklo i ogledala ostaju bez pruga — i bez vlakana.',
  ),
  array(
    'questioon' => 'Kako se najbolje pere?',
    'answer'    => 'Strojno na 40 °C, s deterdžentom bez omekšivača (omekšivač začepi vlakna i smanji upijanje). Ne koristite izbjeljivač i ne sušite u sušilici — sušite na zraku.',
  ),
  array(
    'questioon' => 'Zašto krpa na nekim snimkama izgleda tamnije?',
    'answer'    => 'Zbog rasvjete. Krpa je tamno siva s crnim rubom; pod jakim svjetlom djeluje svjetlije, a u zatvorenom tamnije.',
  ),
  array(
    'questioon' => 'Je li dobra kao poklon?',
    'answer'    => 'Jest — paketi 3+3 i 8+4 su među najčešćim darovima za useljenje i blagdane. Krpa dolazi uredno složena i spremna za darivanje.',
  ),
);
...
$bra_faq = array(
  array(
    ...
    'answer'    => 'Izmjerite opseg ispod grudi i pronađite ga u tablici veličina zajedno s vašom košaricom. Ako ste između dvije veličine, uzmite veću — grudnjak je bešavan i lagano se prilagođava tijelu.',
  ),
  array(
    'questioon' => 'Ima li žicu?',
    'answer'    => 'Nema. Potpora dolazi iz čvrste trake ispod grudi i oblikovanih košarica, pa nema žice koja pritišće rebra ili probija tkaninu.',
  ),
  array(
    'questioon' => 'Kako radi korekcija držanja?',
    'answer'    => 'Prekriženi dizajn na leđima nježno povlači lopatice jednu prema drugoj i odvraća od pogrbljenog položaja. To je podsjetnik tijelu, a ne kruta ortoza — pokret ostaje potpuno slobodan.',
  ),
  array(
    'questioon' => 'Vidi se li ispod odjeće?',
    'answer'    => 'Košarice su bešavne i glatke, a rubovi mekani, pa se pod majicom i pripijenom haljinom ne ocrtava. Bež nijansa je najmanje vidljiva ispod svijetle odjeće.',
  ),
  array(
    'questioon' => 'Koliko razina zategnutosti ima kopča?',
    'answer'    => 'Prednja kopča ima dvije kopče i ukupno 8 razina, pa zategnutost prilagodite tijekom dana ili kroz ciklus.',
  ),
  array(
    'questioon' => 'Kako ga prati?',
    'answer'    => 'Ručno pranje ili strojno na 30 °C u vrećici za rublje, bez omekšivača i bez sušilice. Tako tkanina zadržava elastičnost.',
  ),
  array(
    'questioon' => 'Mogu li ga nositi za vrijeme sna ili treninga?',
    'answer'    => 'Možete ga nositi za spavanje i za laganu rekreaciju jer nema žice. Za intenzivan trening preporučujemo pravi sportski grudnjak s većom razinom potpore.',
  ),
  array(
...
// NORIKS HYD (orto-hyd) — product FAQ za "Informacije o Proizvodu".
$hyd_faq = array(
  array(
    'questioon' => 'Što je vodikova voda?',
    'answer'    => 'To je obična pitka voda u kojoj je otopljen molekularni vodik (H₂). Vodik je najmanja molekula i u istraživanjima se povezuje sa selektivnim antioksidativnim djelovanjem.',
  ),
  array(
    'questioon' => 'Koliko traje jedan ciklus?',
    'answer'    => 'Kratki ciklus traje 3 minute i daje oko 1.600 ppb vodika, a dugi 10 minuta i daje do 3.000 ppb. Ciklus odaberete pritiskom na tipku.',
  ),
  array(
    'questioon' => 'Dodiruje li voda plastiku?',
    'answer'    => 'Ne. Boca je od borosilikatnog stakla s bazom od nehrđajućeg čelika, pa u vodi nema plastičnog okusa ni čestica.',
  ),
  array(
    'questioon' => 'Kakvu vodu mogu koristiti?',
    'answer'    => 'Bilo koju pitku vodu — iz slavine, filtriranu, flaširanu ili destiliranu. Zahvaljujući PEM/SPE membrani uređaj ne treba minerale da bi radio.',
  ),
  array(
    'questioon' => 'Treba li mijenjati filter?',
    'answer'    => 'Ne treba. Generator ima trajni filter, pa nema potrošnog materijala ni dodatnih troškova.',
  ),
  array(
    'questioon' => 'Kako se puni?',
    'answer'    => 'USB kabelom; puno punjenje traje 30 do 60 minuta i dovoljno je za više ciklusa. Bocu ne punite dok je u njoj voda.',
  ),
  array(
    'questioon' => 'Kada popiti vodu?',
    'answer'    => 'Najbolje odmah nakon ciklusa, unutar 30-ak minuta, jer koncentracija vodika s vremenom pada.',
  ),
  array(
...
$kneefix_faq = array(
  ...
  array(
    'questioon' => 'Kako odabrati veličinu?',
    'answer'    => 'Veličine su određene prema tjelesnoj težini: M (50–75 kg), L (76–90 kg), XL (91–110 kg) i 2XL (110 kg+).',
  ),
);
...
// NORIKS Cards (zvucni uredaj s karticama) — FAQ za "Informacije o Proizvodu".
$noriks_cards_faq = array(
  array(
    'questioon' => 'Za koju je dob namijenjen?',
  ...
  array(
    ...
    'answer'    => 'Ne. Uređaj radi samostalno — bez ekrana, aplikacije i interneta. Umetnete karticu, pritisnete gumb i uređaj izgovori riječ.'
  ),
  array(
    'questioon' => 'Može li ga dijete koristiti samo?',
    'answer'    => 'Da, upravo je tako i zamišljen. Kartica se jednostavno umetne, gumb je velik i jasno označen, a uređaj je lagan i bez oštrih rubova.'
  ),
  array(
    'questioon' => 'Kako se puni i koliko traje baterija?',
    'answer'    => 'Uređaj se puni priloženim kabelom. Uz svakodnevnu upotrebu punjenje je obično potrebno svakih nekoliko dana.'
  ),
  array(
    'questioon' => 'Jesu li kartice izdržljive?',
    'answer'    => 'Kartice su tiskane na debljem, ojačanom materijalu otpornom na trganje i savijanje, a površina se lako obriše vlažnom krpom.'
  ),
  array(
    'questioon' => 'Može li se glasnoća smanjiti?',
    'answer'    => 'Da, glasnoća se podešava na uređaju — dovoljno glasno za dnevni boravak, tiše za večernju igru prije spavanja.'
  ),
);
...
$dental_faq = array(
  ...
  array(
    'questioon' => 'Kako uređaj radi?',
    'answer'    => 'Ultrazvučni valovi od <strong>43.000 Hz</strong> stvaraju milijune mikroskopskih mjehurića koji uklanjaju naslage i mrlje i s mjesta do kojih četkica ne dolazi. Istovremeno ugrađeno UV-C svjetlo uklanja do 99,9 % bakterija, virusa i gljivica — sve u 3 minute, bez ribanja i bez agresivnih kemikalija.'
  ),
  array(
    'questioon' => 'Je li siguran za svakodnevnu upotrebu?',
    'answer'    => 'Jest. Namijenjen je svakodnevnom čišćenju i siguran je za sva zubna pomagala, uključujući prozirne folije koje se lako izgrebu četkicom ili pastom.'
  ),
  array(
    'questioon' => 'Moram li koristiti tablete za čišćenje?',
    'answer'    => 'Ne. Uređaj radi sa samom hladnom vodom. Kod tvrdokornih mrlja ili mirisa možete dodati tabletu, ali ona nije uvjet za čist rezultat.'
  ),
  array(
  ...
  array(
    ...
    'answer'    => 'Na izbor su ciklusi od 3 minute (svakodnevno) i 5 minuta (dubinsko čišćenje). Uređaj radi tiho, do 65 dB — otprilike kao tiši razgovor.'
  ),
  array(
  ...
  array(
    ...
    'answer'    => 'Jest. Uređaj je laboratorijski testiran na <em>E. coli</em>, <em>Staphylococcus aureusu</em> i <em>Candidi albicans</em>, uz stopu uništenja veću od 99,9 % u sva tri ponavljanja testa.'
  ),
);

// NORIKS HERS HairMagic+ — FAQ za "Informacije o Proizvodu" (po originalu).
$hairmagic_faq = array(
  array(
    'questioon' => 'Kako se nanosi?',
    'answer'    => 'Na čistu i suhu kosu. Kistom iz pakiranja utapkajte puder na liniju kose, razdjeljak ili prorijeđena mjesta i lagano rasporedite. Cijeli postupak traje manje od dvije minute.'
  ),
  array(
    'questioon' => 'Kako odabrati pravu nijansu?',
    'answer'    => 'Odaberite nijansu najbližu vašoj boji kose. Ako ste između dvije, uzmite <strong>svjetliju</strong> — puder je nadogradiv, pa tamniji ton uvijek možete dodati u još jednom sloju.'
  ),
  array(
    'questioon' => 'Koliko traje jedno pakiranje?',
    'answer'    => 'Uz svakodnevnu upotrebu obično 2 do 4 mjeseca, ovisno o veličini površine koju prekrivate.'
  ),
  array(
    'questioon' => 'Drži li po kiši i znoju?',
    'answer'    => 'Da, formula je otporna na vodu i prijenos — ne razmazuje se i ne prlja ovratnik ni jastučnicu. Skida se običnim pranjem kose šamponom.'
  ),
  array(
    'questioon' => 'Je li prikladan za osjetljivo vlasište?',
    'answer'    => 'Jest. Formula je obogaćena arganovim uljem, panthenolom i vitaminom E te je namijenjena i osjetljivom vlasištu. Ne izaziva svrbež ni osjećaj zatezanja.'
  ),
  array(
    'questioon' => 'Mogu li ga koristiti na obrvama?',
    'answer'    => 'Da. Malom količinom popunite praznine u obrvama — rezultat je prirodan, bez oštrih linija kao kod olovke.'
  ),
  array(
    'questioon' => 'Vidi li se da je nešto naneseno?',
    'answer'    => 'Ne. Prilagodljivi pigmenti stapaju se s vašom bojom kose, tekstura je lagana i ne stvara naslage. Kosa se i dalje normalno pomiče i češlja.'
  ),
);

// NORIKS FaceLift — FAQ za "Informacije o Proizvodu" (po originalu).
$lift_faq = array(
  array(
    'questioon' => 'Kada ću vidjeti definiraniju liniju čeljusti?',
    'answer'    => 'Većina korisnica primijeti smanjenje otekline i privremeno podizanje već nakon prve primjene od 20 minuta. Vidljivija, trajnija promjena obično dolazi kroz <strong>4 do 8 tjedana</strong> redovite uporabe.'
  ),
  array(
    'questioon' => 'Što je kolagen od 300 daltona i zašto je bolji?',
    'answer'    => 'Obične molekule kolagena prevelike su da prođu površinski sloj kože. Molekula od 300 daltona dovoljno je mala da uđe dublje — zato tkanina radi na strukturi kože, a ne samo na njezinoj površini.'
  ),
  array(
    'questioon' => 'Koliko dugo se nosi?',
    'answer'    => '<strong>20 do 30 minuta dnevno</strong> — navečer, dok gledate TV ili završavate njegu lica. Duže nošenje ne ubrzava rezultat; redovitost je ta koja radi razliku.'
  ),
  array(
    'questioon' => 'Odgovara li svima?',
    'answer'    => 'Da. Traka je univerzalne veličine, a čičak-trake na tjemenu i zatiljku prilagode je različitim oblicima glave i lica. Otvori za uši drže je na mjestu.'
  ),
  array(
    'questioon' => 'Mogu li je koristiti uz svoju njegu lica?',
    'answer'    => 'Svakako. Serum ili kremu nanesite prije stavljanja — kompresija pomaže da se sastojci bolje upiju i zadrži vlaga.'
  ),
  array(
    'questioon' => 'Djeluje li na podbradak i opuštene obraze?',
    'answer'    => 'Traka je oblikovana upravo za donji dio lica i područje ispod brade. Kompresija radi protiv sile teže i podupire tkivo prema gore.'
  ),
  array(
    'questioon' => 'Koliko puta se može koristiti?',
    'answer'    => 'Jedna traka izdrži do <strong>15 uporaba</strong> pune učinkovitosti. Periva je i brzo se suši, pa je spremna za sljedeću večer.'
  ),
  array(
    'questioon' => 'Mogu li je nositi nakon estetskog zahvata?',
    'answer'    => 'Mnoge korisnice je nose kao potporu nakon zahvata na bradi ili licu. Uvijek se prvo posavjetujte sa svojim liječnikom i pričekajte da koža zacijeli.'
  ),
);
...
// NORIKS Pre — FAQ za "Informacije o Proizvodu" (po originalu).
$pre_faq = array(
  array('questioon' => 'Kako jastuk djeluje?',
    'answer'    => 'Dva krila obuhvate trbuščić sprijeda i leđa straga, pa tijelo ostaje na boku. Duljinu prilagodite tako da jastuk udobno nalijegne, a manji jastučić dodate ondje gdje vam treba više potpore.'),
  array('questioon' => 'Čemu služi mali jastučić?',
    'answer'    => 'Dodaje potporu pod trbuščić ili iza leđa tako da se zaglavi između vas i velikog jastuka. Možete ga staviti i <strong>među koljena</strong> umjesto zasebnog jastuka za noge.'),
  array('questioon' => 'Od čega je napravljen?',
    'answer'    => 'Navlaka je od mješavine <strong>bambusa i poliestera</strong> — prozračna i mekana. Punjenje je od poliestera i mikrovlakana koje bolje zadržava oblik od običnog punjenja.'),
  array('questioon' => 'Kako se pere?',
    'answer'    => 'Navlaka se skida i pere u perilici — za grinje perite na <strong>60 °C</strong> i potom osušite u sušilici da jastuk ostane pun. Nije potrebno kemijsko čišćenje.'),
  array('questioon' => 'Zauzima li puno mjesta u krevetu?',
    'answer'    => 'Ne. Za razliku od velikih C i U jastuka, NORIKS Pre je kompaktan — partner ostaje u krevetu, a okretanje na drugu stranu ne traži preslagivanje.'),
  array('questioon' => 'Od kojeg tjedna trudnoće se koristi?',
    'answer'    => 'Možete ga koristiti čim spavanje na leđima postane neudobno. Duljina se prilagođava, pa jastuk prati rast trbuščića kroz cijelu trudnoću.'),
...
$snug_faq = array(
  array('questioon' => 'Koje su dimenzije jastuka?',
    'answer'    => 'Jedna veličina: <strong>105 cm duljine i 30 cm širine</strong>. Podupire vas od ramena do koljena, a ne zauzima cijeli krevet — zato je lako držati ga i lako se okrenuti s njim.'),
  array('questioon' => 'Hoće li se s vremenom sliježati?',
    'answer'    => 'Ne. Punjenje je od tisuća finih, elastičnih vlakana koja se vraćaju u oblik. Potpora koju osjetite prve noći ista je i nakon devedeset noći.'),
  array('questioon' => 'Čime je punjen?',
    'answer'    => 'Visokoelastičnim vlaknastim punjenjem — mekano i ugodno izvana, čvrsto i potporno iznutra. Bez memorijske pjene koja se pregrijava.'),
  array('questioon' => 'Kako se pere?',
    'answer'    => 'Navlaka se skida i pere u perilici na 40 °C. Sam jastuk ne perite u stroju — po potrebi ga prozračite i osušite na zraku.'),
  array('questioon' => 'Je li prikladan u trudnoći?',
    'answer'    => 'Jest. S-oblik podupire trbuh sprijeda i leđa straga, a preporučeni položaj u trudnoći je spavanje na lijevoj strani. Kod zdravstvenih komplikacija posavjetujte se s liječnikom.'),
  array('questioon' => 'Koliko treba da se naviknem?',
    'answer'    => 'Većina korisnika nađe svoj položaj do druge noći. S-oblik je drukčiji od ravnog jastuka, pa je prvih par noći tijelo uči gdje se smjestiti.'),
  array('questioon' => 'Koje boje su dostupne?',
    'answer'    => 'Šest boja: plava, roza, siva, zelena, ljubičasta i tamnoplava. Boju birate na ovoj stranici prije dodavanja u košaricu.'),
...
$kompwom_faq = array(
  array('questioon' => 'Kako odabrati veličinu?',
    'answer'    => 'Idite prema opsegu grudi — on određuje kako majica sjeda na prsa i ramena. Ako ste između dvije veličine, uzmite <strong>veću</strong>. Dostupne su veličine od S do 3XL.'),
  array('questioon' => 'Vidi li se ispod odjeće?',
    'answer'    => 'Ne. Pletivo je bešavno, tanko i mat, pa nestane ispod košulje, sakoa ili pripijene haljine. Nema ruba koji bi se ocrtavao.'),
  array('questioon' => 'Rola li se tijekom dana?',
    'answer'    => 'Ne. Kompresija je raspoređena po širini umjesto da stišće u jednoj točki, pa majica ostaje na mjestu i nakon cijelog dana.'),
  array('questioon' => 'Jesu li 3D linije tiskane?',
    'answer'    => 'Nisu. Reljef je <strong>upleten u samu tkaninu</strong>, pa ništa ne puca i ništa se ne ljušti s vremenom, koliko god puta je oprali.'),
  array('questioon' => 'Koliko čvrsto steže?',
    'answer'    => 'Čvrsto, ali nikad tijesno. Trebate normalno disati i jesti bez razmišljanja o majici. Ako je trag na koži vidljiv dvadeset minuta nakon skidanja, veličina je premala.'),
  array('questioon' => 'Kako se pere?',
    'answer'    => 'U stroju na <strong>30 °C</strong>. Bez izbjeljivača, bez glačanja i bez sušilice — sušite na zraku.'),
  array('questioon' => 'Koje boje su dostupne?',
    'answer'    => 'Tri boje: crna, tamnosiva i roza. Boju i veličinu birate na ovoj stranici prije dodavanja u košaricu.'),
  array('questioon' => 'Mogu li je vratiti?',
    'answer'    => 'Da, imate <strong>30 dana</strong> za povrat novca ili zamjenu veličine. Dovoljan je e-mail, bez obrazaca.'),
);

$pal_faq = array(
  array('questioon' => 'Čemu služi druga ručka?',
    'answer'    => 'Za <strong>ustajanje</strong>. Donju ručku uhvatite kad ustajete iz fotelje, s kreveta ili s niske stolice — pritisak ide okomito prema dolje, pa se ne morate naginjati naprijed ni tražiti tuđu pomoć.'),
  array('questioon' => 'Stoji li stvarno sam?',
    'answer'    => 'Da. Baza ima <strong>četiri gumene nožice</strong> koje drže štap uspravno kad ga pustite. Ne pada na pod, pa se ne morate saginjati da biste ga podigli.'),
  array('questioon' => 'Kliže li po glatkim podovima?',
    'answer'    => 'Ne. Nožice su od protuklizne gume i drže na pločicama, parketu i laminatu. Baza se prilagodi i neravnom terenu vani.'),
  array('questioon' => 'Kako radi svjetiljka?',
    'answer'    => 'Svjetiljka je ugrađena u ručku i uključuje se gumbom. Osvjetljava put pred vama — za odlazak do kupaonice noću ili šetnju u sumrak.'),
  array('questioon' => 'Što radi alarm?',
    'answer'    => 'Pritiskom na gumb oglašava se <strong>glasan zvučni signal</strong> koji upozori ukućane ako padnete ili vam zatreba pomoć.'),
  array('questioon' => 'Podesiva je visina?',
    'answer'    => 'Jest. Visina se podešava u nekoliko sekundi, bez alata, pa štap odgovara svim visinama korisnika.'),
  array('questioon' => 'Je li sklopiv?',
    'answer'    => 'Jest. Sklopi se u nekoliko dijelova i stane u torbu ili pretinac u autu — praktično za putovanja i odlaske k liječniku.'),
  array('questioon' => 'Mogu li ga vratiti?',
    'answer'    => 'Da, imate <strong>30 dana</strong> za povrat novca ili zamjenu. Dovoljan je e-mail, bez obrazaca.'),
);
...
$home_faq = array(
  ...
  array('questioon' => 'Koliko drže?',
    'answer'    => 'Svaka kuka nosi <strong>do 7 kg</strong> — mokri ručnik, kupaći ogrtač ili torbu. U pakiranju su četiri kuke.'),
  array('questioon' => 'Treba li bušiti ili lijepiti?',
    'answer'    => 'Ne. Kuka se postavlja <strong>bez bušenja, vijaka i ljepila</strong> — prislonite je i zavrnete gumb. Montaža traje nekoliko sekundi.'),
  array('questioon' => 'Kako se postavlja?',
    'answer'    => 'Očistite i osušite površinu, prislonite kuku i zavrnete gumb do kraja. Preporučujemo da prvi dan gumb <strong>još jednom zategnete</strong> i tek onda objesite teži predmet.'),
  array('questioon' => 'Ostavlja li tragove kad je skinem?',
    'answer'    => 'Ne. Otpustite gumb i kuka se skida <strong>bez traga</strong> — nema ostataka ljepila ni oštećenja, pa je prikladna i za iznajmljeni stan.'),
  array('questioon' => 'Mogu li je premjestiti?',
    'answer'    => 'Da. Kuka je <strong>višekratna</strong> — skinete je, obrišete brtvu i postavite na novo mjesto.'),
  array('questioon' => 'Hrđa li u vlažnoj kupaonici?',
    'answer'    => 'Ne. Kućište je od <strong>izdržljivog polimera otpornog na hrđu</strong> i vlagu, izrađeno za svakodnevnu upotrebu u kupaonici.'),
  array('questioon' => 'Koje su dimenzije?',
    'answer'    => 'Promjer <strong>6,1 cm</strong>, dubina <strong>4 cm</strong>. Dostupne nijanse: crna, krom, bijela, zlatna i siva.'),
  array('questioon' => 'Mogu li ih vratiti?',
    'answer'    => 'Da, imate <strong>30 dana</strong> za povrat novca ili zamjenu. Dovoljan je e-mail, bez obrazaca.'),
);

$seal_faq = array(
  array('questioon' => 'Koliko dugo hrana ostaje svježa?',
    'answer'    => 'Ovisi o namirnici, ali bez zraka nema oksidacije — <strong>meso, riba, sir i povrće ostaju svježi znatno dulje</strong> nego u običnoj vrećici ili posudi s poklopcem. Avokado ne posmeđi preko noći, jagode ne uplijesne za dva dana.'),
  array('questioon' => 'Kako se koristi?',
    'answer'    => 'U četiri koraka: napunite vrećicu, zatvorite zatvarač, prislonite uređaj na ventil i pritisnite gumb. Uređaj se <strong>sam isključi</strong> kad je zrak vani.'),
  array('questioon' => 'Koliko traje baterija?',
    'answer'    => 'Punjiva baterija od <strong>1200 mAh</strong> dovoljna je za više stotina vrećica na jedno punjenje. Puni se preko USB-C kabela, istog kao za mobitel.'),
  array('questioon' => 'Što je sve u pakiranju?',
    'answer'    => 'Uređaj, kabel za punjenje, klipsa za zatvaranje vrećica, upute i vakuumske vrećice. <strong>Polovica vrećica je manja (1 l), polovica veća (2 l)</strong>.'),
  array('questioon' => 'Mogu li se vrećice prati i koristiti ponovno?',
    'answer'    => 'Da. Vrećice su <strong>za višekratnu uporabu</strong> i sigurne su za zamrzivač, mikrovalnu i perilicu posuđa.'),
  array('questioon' => 'Radi li i s posudama?',
    'answer'    => 'Da, uz NORIKS posude s ventilom. Postupak je isti kao s vrećicom — uređaj prislonite na ventil na poklopcu.'),
  array('questioon' => 'Koliko je velik?',
    'answer'    => 'Dug je <strong>20,5 cm</strong>, širok 4,7 cm i težak 237 g — stane u ladicu za pribor i radna ploha ostaje prazna.'),
  array('questioon' => 'Mogu li ga vratiti?',
    'answer'    => 'Da, imate <strong>30 dana</strong> za povrat novca ili zamjenu. Dovoljan je e-mail, bez obrazaca.'),
);

$sr_faq = array(
  array('questioon' => 'Stvarno se ne gužva?',
    'answer'    => 'Tako je. Tkanina s elastičnošću u četiri smjera <strong>vraća se u svoj oblik</strong>, pa košulja izgleda uredno cijeli dan — i kad je izvadite iz torbe. Glačanje nije potrebno.'),
  array('questioon' => 'Jesu li veličine točne?',
    'answer'    => 'Da, košulja je vjerna veličinama. U akordeonu „Tablica veličina“ imate visine i težine modela s fotografija. <strong>Ako ste između dvije veličine, uzmite veću</strong> za opušteniji kroj.'),
  array('questioon' => 'Koji rukavi i boje su dostupni?',
    'answer'    => 'Birate <strong>kratki ili dugi rukav</strong>, u 8 boja: bijela, svijetloplava, crna, teget, grafitna, bordo, petrol i tamno petrol. Veličine od S do 3XL.'),
  array('questioon' => 'Od čega je napravljena?',
    'answer'    => 'Od mješavine poliestera i elastana s <strong>12 % elastana</strong> — lagana, mekana i prozračna tkanina koja odvodi vlagu i toplinu s kože.'),
  array('questioon' => 'Kako se pere?',
    'answer'    => 'Strojno na <strong>programu za osjetljivo rublje</strong>, na 30 °C. Sušite na niskoj temperaturi ili na zraku kako tkanina zadrži elastičnost i ne bi se skupila.'),
  array('questioon' => 'Ima li džep na prsima?',
    'answer'    => 'Nema. Kroj je čist, s klasičnim raširenim ovratnikom i zakopčavanjem sprijeda — <strong>bez džepa na prsima</strong>.'),
  array('questioon' => 'Za koje prilike je prikladna?',
    'answer'    => 'Za ured, sastanke, večere, proslave i putovanja. Izgleda uredno bez glačanja, pa je praktična i za <strong>cjelodnevno nošenje</strong>.'),
  array('questioon' => 'Mogu li je vratiti ili zamijeniti?',
    'answer'    => 'Da, imate <strong>30 dana</strong> za povrat novca ili zamjenu veličine. Dovoljan je e-mail, bez obrazaca.'),
);

$hug_faq = array(
  array('questioon' => 'Koliko dugo ostaje topao?',
    'answer'    => 'Navlaka ima <strong>dvostruki sloj izolacije</strong>, pa toplina ostaje ugodna satima — bez naglog skoka od prevruće do mlake. Krakovi drže bocu uz tijelo, zato grije osjetno duže od klasičnog termofora.'),
  array('questioon' => 'Dolazi li s bocom?',
    'answer'    => 'Da. U pakiranju su nosiva navlaka i <strong>termofor od 500 ml</strong> — ne trebate ništa dokupljivati.'),
  array('questioon' => 'Sadrži li boca lateks?',
    'answer'    => 'Ne. Boca je izrađena od <strong>100 % prirodne gume</strong> i ne sadrži lateks.'),
  array('questioon' => 'Što ako mi ne odgovara opseg?',
    'answer'    => 'Hugger odgovara opsegu struka <strong>70 – 93 cm</strong>. Za veći opseg postoji produžetak veličine koji ga povećava do 110 cm.'),
  array('questioon' => 'Kako se pere?',
    'answer'    => 'Mrlje obrišite vlažnom krpom. Strojno pranje na najviše <strong>30 °C</strong>, program za osjetljivo rublje i blagi deterdžent. Ne bijeliti, ne peglati, ne sušiti u sušilici. Za najduži vijek preporučujemo ručno pranje u hladnoj vodi.'),
  array('questioon' => 'Kako se puni boca?',
    'answer'    => 'Vrućom, ali <strong>ne kipućom</strong> vodom, do otprilike dvije trećine. Istisnite zrak i dobro zavrnite čep prije nego bocu umetnete u navlaku.'),
...
$slim_faq = array(
  array('questioon' => 'Kako odabrati veličinu?',
    'answer'    => 'Izmjerite <strong>opseg struka</strong> na najužem dijelu trupa, malo iznad pupka, i usporedite s tablicom veličina (XS do 5XL). Ako ste između dvije veličine, uzmite <strong>veću</strong>.'),
  array('questioon' => 'Rolaju li se prema dolje?',
    'answer'    => 'Ne. Široki ukriženi pojas prelazi oko cijelog struka — sprijeda, sa strane i straga — pa se pritisak raspoređuje umjesto da stišće u jednoj liniji. Gaćice ostaju na mjestu i kad sjedite.'),
  array('questioon' => 'Vide li se ispod odjeće?',
    'answer'    => 'Ne. Pletivo je tanko i glatko, bez debelih šavova, pa nestanu ispod pripijene haljine, traperica i tajica.'),
  array('questioon' => 'Nosim li ih umjesto gaćica ili preko njih?',
    'answer'    => 'Kako vam je udobnije. Većina ih nosi same, kao obično donje rublje, a možete ih nositi i preko tankih gaćica.'),
  array('questioon' => 'Mogu li ih nositi nakon poroda?',
    'answer'    => 'Mnoge mame ih nose nakon poroda jer su mekane i prozračne. Nakon carskog reza ili ako niste sigurni, prije nošenja se posavjetujte sa svojim liječnikom.'),
  array('questioon' => 'Koliko čvrsto stežu?',
    'answer'    => 'Čvrsto, ali ne tijesno. Trebate normalno disati i sjediti. Ako vas negdje žuljaju ili ostavljaju dubok trag, uzmite veću veličinu.'),
  array('questioon' => 'Kako se peru?',
    'answer'    => 'Ručno ili u stroju na <strong>30 °C</strong> u vrećici za rublje. Bez izbjeljivača, glačanja i sušilice — sušite na zraku.'),
  array('questioon' => 'Koje boje su dostupne?',
    'answer'    => 'Šest boja: crna, boja kože, tamnoplava, lila, fuksija i šljiva. Boju i veličinu birate na ovoj stranici prije dodavanja u košaricu.'),
  array('questioon' => 'Mogu li ih vratiti?',
    'answer'    => 'Da, imate <strong>30 dana</strong> za povrat novca ili zamjenu veličine. Dovoljan je e-mail, bez obrazaca.'),
);

$bowl_faq = array(
  array('questioon' => 'Za koje pse je zdjelica prikladna?',
    'answer'    => 'Za <strong>male pasmine</strong> — jazavčar, francuski buldog, šnaucer, mops, maltezer i slične. Zdjela ima promjer 15 cm i zapreminu oko 430 ml.'),
  array('questioon' => 'Čemu služi nagib od 15°?',
    'answer'    => 'Zdjela je nagnuta prema psu, pa hrana klizi prema njemu, a glava i vrat su u prirodnijem položaju. Pas ne mora gurati njušku u kut zdjele niti savijati vrat do poda.'),
  array('questioon' => 'Zašto je povišena?',
    'answer'    => 'Postolje od 11 cm podigne zdjelu bliže visini psa, pa jede stojeći, s manje savijanja vrata i leđa. To je posebno ugodno starijim psima.'),
  array('questioon' => 'Mogu li je koristiti za vodu?',
    'answer'    => 'Da. Zdjela je od nehrđajućeg čelika i jednako je prikladna za hranu i vodu. Mnogi vlasnici uzmu dvije — jednu za hranu, drugu za vodu.'),
  array('questioon' => 'Kako se čisti?',
    'answer'    => 'Zdjelu isperite toplom vodom sa sredstvom za pranje posuđa. Glatki čelik ne upija mirise i ne zadržava ostatke hrane kao plastika.'),
  array('questioon' => 'Klizi li po podu?',
    'answer'    => 'Postolje je stabilno i ne prevrće se kad pas jede. Na vrlo glatkim podovima preporučujemo podložak ispod postolja.'),
  array('questioon' => 'Mogu li je vratiti?',
    'answer'    => 'Da, imate <strong>30 dana</strong> za povrat novca. Dovoljan je e-mail, bez obrazaca.'),
);
...
$stepcloud_faq = array(
  ...
  array('questioon' => 'Koliko dugo traju?',
    'answer'    => 'Uz svakodnevno nošenje zadržavaju oblik oko <strong>2 mjeseca</strong>, ovisno o težini i satima na nogama. Zato ih mnogi naruče u paketu od 2 ili 3 para.'),
...
$faq_pick = function( $title, $list ) use ( $is_relief, $relief_faq, $is_gelseat, $gelseat_faq, $is_kneetape, $kneetape_faq, $is_celleg, $celleg_faq, $is_stepcloud, $stepcloud_faq, $is_slim, $slim_faq, $is_bowl, $bowl_faq, $is_snug, $snug_faq, $is_kompwom, $kompwom_faq, $is_pal, $pal_faq, $is_red, $red_faq, $is_snore, $snore_faq, $is_cloud, $cloud_faq, $is_cloath, $cloath_faq, $is_bra, $bra_faq, $is_hyd, $hyd_faq, $is_dental, $dental_faq, $is_hairmagic, $hairmagic_faq, $is_lift, $lift_faq, $is_kneeheat, $kneeheat_faq, $is_pre, $pre_faq, $is_hug, $hug_faq, $is_seal, $seal_faq, $is_home, $home_faq, $is_sr, $sr_faq, $is_brush, $brush_faq, $is_noriks_cards, $noriks_cards_faq, $is_controlpro, $controlpro_faq, $is_kneefix, $kneefix_faq, $is_knc, $knc_faq, $is_ortopas, $ortopas_faq, $is_bunion, $bunion_faq, $is_fisiorest, $fisiorest_faq, $is_norikshers, $norikshers_faq, $is_leakboxers, $leakboxers_faq, $is_kompmajice, $kompmajice_faq, $is_jastuk, $jastuk_faq, $is_nosilka, $nosilka_faq, $is_kidsnest, $kidsnest_faq ) {
  ...
  if ( $is_dental && $is_info ) {
    return $dental_faq;
  }
  if ( $is_hairmagic && $is_info ) {
    return $hairmagic_faq;
  }
  if ( $is_lift && $is_info ) {
    return $lift_faq;
  }
