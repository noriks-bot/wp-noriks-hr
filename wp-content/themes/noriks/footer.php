...
<!-- suport section -->
	...
	if ( ! $nf_brand ) {
		$nf_brand = 'NORIKS je europska marka proizvoda za svakodnevnu upotrebu. Usredotočeni smo na kvalitetu, udobnost i poštene cijene — vjeruje nam već više od 120.000 kupaca.';
	}
		...
		<section class="nf-signup">
			<div class="nf-wrap nf-signup-grid">
				<div class="nf-signup-copy">
					<h2 class="nf-signup-title">Pridruži se NORIKS obitelji</h2>
					<p class="nf-signup-sub">Ekskluzivne ponude i najave novih proizvoda — prvi doznaj.</p>
				</div>
				<div class="nf-signup-form-col">
					<form class="nf-signup-form" data-klaviyo-list="<?php echo esc_attr( $nf_klaviyo_list ); ?>" novalidate>
						...
						<p class="nf-signup-note">
							Prijavom pristaješ na primanje e-mailova. Odjaviti se možeš u svakom trenutku.
						...
						<div class="nf-link-col">
							...
							if ( $nf_col2_l ) :
								foreach ( $nf_col2_l as $item ) :
									if ( false !== strpos( (string) $item['link'], '/upute' ) ) { $nf_has_upute = true; }
									?>
								<a href="<?php echo esc_url( $item['link'] ); ?>"><?php echo esc_html( $item['text'] ); ?></a>
							<?php endforeach; endif; ?>
							<?php if ( ! $nf_has_upute ) : ?>
								<a href="/hr/upute/">Upute za uporabu</a>
							<?php endif; ?>
						</div>
					<?php endif; ?>
		...
		<div class="nf-wrap nf-meta">
			<div class="nf-meta-left">
				<span class="nf-region">
					<img loading="lazy" decoding="async" src="https://static.devit.software/countries/flags/rectangle/hr.svg" alt="" class="nf-flag">
					Hrvatska (HR)
				</span>
				<span class="nf-copy">© <?php echo date( 'Y' ); ?> NORIKS. Sva prava pridržana.</span>
			</div>
	...
	(function () {
		...
		form.addEventListener('submit', function (e) {
			...
			if (!email || email.indexOf('@') === -1) {
				msg.textContent = 'Unesi ispravnu e-mail adresu.'; msg.className = 'nf-signup-msg is-error'; return;
			}
			if (!list || list === 'REPLACE_LIST_ID') {
				msg.textContent = 'Newsletter još nije povezan (Klaviyo List ID nedostaje).'; msg.className = 'nf-signup-msg is-error'; return;
			}
			...
			fetch('https://manage.kmail-lists.com/ajax/subscriptions/subscribe', {
				...
				if (d && d.success) {
					msg.textContent = 'Hvala! Provjeri e-mail za potvrdu.'; msg.className = 'nf-signup-msg is-ok';
					form.reset();
				} else {
					msg.textContent = (d && d.errors && d.errors[0]) ? d.errors[0] : 'Nešto je pošlo po zlu, pokušaj ponovno.';
					msg.className = 'nf-signup-msg is-error';
				}
			}).catch(function () {
				msg.textContent = 'Nešto je pošlo po zlu, pokušaj ponovno.'; msg.className = 'nf-signup-msg is-error';
			});
		});
	})();
	</script>
