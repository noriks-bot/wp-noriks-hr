<?php
/**
 * Checkout Form — Vigoshop 1:1 replica with WooCommerce functionality
 * Matches /test-checkout/ standalone template exactly
 */
...
<form name="checkout" method="post" class="checkout woocommerce-checkout"
  ...
  <!-- Hidden WC #payment — needed for WC AJAX update_order_review + form submission -->
  ...
  <!-- Visible payment UI — 1:1 vigoshop HTML -->
  <div id="noriks-payment">
    <ul class="wc_payment_methods payment_methods methods">
      <li class="wc_payment_method payment_method_cod">
        <input id="payment_method_cod_visible" type="radio" class="input-radio" name="noriks_payment" value="cod" checked='checked' data-order_button_text="">
        <label for="payment_method_cod_visible">
      ...
      <li class="wc_payment_method payment_method_braintree_credit_card">
        <input id="payment_method_card_visible" type="radio" class="input-radio" name="noriks_payment" value="stripe" data-order_button_text="Naruči">
        <label for="payment_method_card_visible">
      ...
      <li class="wc_payment_method payment_method_braintree_paypal">
        <input id="payment_method_paypal_visible" type="radio" class="input-radio" name="noriks_payment" value="ppcp-gateway" data-order_button_text="Naruči">
        <label for="payment_method_paypal_visible">
...
<!-- ORDER REVIEW with submit button — vigoshop structure -->
<div id="order_review" class="woocommerce-checkout-review-order container container--xs bg--white">
  <button type="submit" class="button alt button--l button--block button--green button--rounded button--green-gradient"
          name="woocommerce_checkout_place_order" id="noriks_place_order"
          onclick="document.getElementById('place_order')&&document.getElementById('place_order').click();return false;"
          data-value="Naruči">Naruči</button>
</div>
...
<script>
/* Dynamic shipping dates (2-5 business days from today) — same as vigoshop */
(function(){
  ...
  var el=document.getElementById('js-delivery-dates');
  if(el) el.textContent=days[from.getDay()]+', '+from.getDate()+'.'+(from.getMonth()+1)+'. - '+days[to.getDay()]+', '+to.getDate()+'.'+(to.getMonth()+1)+'.';
})();

/* Sync visible payment radios → hidden WC #payment radios */
(function(){
  var map = {'cod':'cod','stripe':'stripe','ppcp-gateway':'ppcp-gateway'};

  function syncPayment(){
    var sel = document.querySelector('#noriks-payment input[name="noriks_payment"]:checked');
    if(!sel) return;
    var val = sel.value;

    // Set hidden WC radio
    var wcRadios = document.querySelectorAll('#payment input[name="payment_method"]');
    wcRadios.forEach(function(r){
      if(r.value === val) {
        r.checked = true;
    ...
    if(prompt) prompt.style.display = (val==='cod') ? '' : 'none';
  }

  document.querySelectorAll('#noriks-payment input[name="noriks_payment"]').forEach(function(r){
