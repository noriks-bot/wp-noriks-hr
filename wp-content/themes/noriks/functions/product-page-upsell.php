 ...
 *
 * Okvir se prikazuje ODMAH ISPOD gumba "Dodaj u košaricu" i nudi 4x Zelene Bokserice
	...
	acf_add_local_field_group( array(
		...
		'fields' => array(
			array(
				...
				'label'        => 'Prikaži upsell ispod gumba (4x Zelene Bokserice)',
				'name'         => 'noriks_pp_upsell',
				'type'         => 'true_false',
				'instructions' => 'Dodaje okvir "Kupi zajedno i uštedi" odmah ispod gumba Dodaj u košaricu. Kupac bira veličinu, a 4 komada se dodaju po upsell cijeni. Vrijedi samo za ovaj proizvod.',
				'ui'           => 1,
			),
			array(
				'key'          => 'field_noriks_pp_upsell2',
				'label'        => 'Prikaži upsell #2 ispod gumba (2 majice: crna + siva)',
			...
			array(
				...
				'label'        => 'Prikaži upsell #3 ispod gumba (NORIKS FIT kompresijska majica)',
				'name'         => 'noriks_pp_upsell3',
				'type'         => 'true_false',
				'instructions' => 'Treći upsell okvir: NORIKS FIT kompresijska majica, kupac bira veličinu. Neovisan o prva dva prekidača.',
				'ui'           => 1,
			),
		),
		'location'   => array(
...
function noriks_pp_upsell2_config() {
	return apply_filters( 'noriks_pp_upsell2_config', array(
		...
		'title'      => '1x Crna + 1x Siva majica',
		'desc'       => 'Dvije osnovne majice u paketu — dodajte ih uz narudžbu uz %s%% popusta.',
		'size_attr'  => 'Veličina',
		'sku'        => 'NORIKS-SHIRTS-BLACK-GRAY-2-PACK-UPSELL',
		'image'      => get_template_directory_uri() . '/img/upsell/upsell-2x-majici.png',
		'label'      => 'Kupi zajedno i uštedi:',
		'add_text'   => 'Dodaj u kupnju',
		'aria'       => 'Veličina majica',
	) );
}
...
function noriks_pp_upsell2_maybe_add( $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data ) {
	...
	$lines  = array(
		( $prod_a ? $prod_a->get_name() : 'Crna majica' ) . ' - ' . $size,
...
function noriks_pp_upsell3_config() {
	return apply_filters( 'noriks_pp_upsell3_config', array(
		'product_id' => 9403,   // NORIKS FIT kompresijska majica (jednostavan proizvod)
		'total'      => 17.99,  // cijena za 1 komad
		'title'      => 'NORIKS FIT kompresijska majica',
		'desc'       => 'Oblikuje siluetu i podupire držanje — dodajte je uz narudžbu uz %s%% popusta.',
		'size_attr'  => 'Veličina',
...
function noriks_pp_upsell3_order_item_meta( $item, $cart_item_key, $values, $order ) {
	if ( ! empty( $values['_noriks_pp_upsell3'] ) ) {
		...
		if ( ! empty( $values['_noriks_pp_upsell_size'] ) ) {
			$item->update_meta_data( 'Veličina', sanitize_text_field( $values['_noriks_pp_upsell_size'] ) );
		}
	}
}
